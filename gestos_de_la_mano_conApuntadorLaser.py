...
def calcula_gestos(puntos):
    ...
    # El taller consiste en modificar las siguientes dos asignaciones: 
    # La primera debe detectar cuando la mano está apuntando
    gestos['mano_apuntando'] = puntos[0,0]
    # La segunda debe colocar la ubicación del puntero, 
    # el cual varia en función del angulo del pulgar.
    # A cero grados, el puntero está en el indice.
    # A medida que aumenta el ángulo, el puntero se aleja. 
    # A noventa grados, el puntero está a cinco veces la longitud del índice. 
    gestos['coordenadas_puntero'] = (puntos[0,8],puntos[1,4])
...
#cap = cv2.VideoCapture('movimiento_03.mp4')  # Captura un 'stream' a partir de un archivo de video.
...
# Dimensiones de mi pantalla 1920 x 1080
screen = pg.display.set_mode((1920,1080)) # Pantalla de nuestro 'Paint'.
    ...
    if results.multi_hand_landmarks: # si se encontraron manos ...
        for handLms in results.multi_hand_landmarks: # para cada mano ...
            ...
            if gesto['mano_abierta'] > 0.9:
                    cv2.circle(img,cv_pos_punto, 5, (255,0,255), -1)
            ...
            # Colocar el puntero     
            if gesto['mano_apuntando'] > 0.5:
                   x_pun,y_pun=gesto['coordenadas_puntero']
                   cv2.circle(img,cv_pos_punto, 5, (0,0,255), -1)
                   pg_pos_punto=(int(varios_pun_np[0,8]*pg_w),int(varios_pun_np[1,8]*pg_h))
                   pg.draw.circle(screen, (  255,   0, 0), pg_pos_punto, 4)
